...
import config
from organizer import _clean_tokens, _normalize_multi_ep_tag, build_final_path, parse_filename
...
# ── _normalize_multi_ep_tag no-match ──


def test_normalize_multi_ep_tag_no_match():
    """Non-matching token is uppercased."""
    assert _normalize_multi_ep_tag("random") == "RANDOM"


# ── _clean_tokens DD5.1 channel digit skip ──


def test_clean_tokens_dd5_digit_skip():
    """DD5 followed by lone digit '1' should both be stripped."""
    result = _clean_tokens(["DD5", "1", "Title"])
    assert result == ["Title"]


# ── _parse_caption blank lines ──


def test_parse_caption_blank_lines():
    """Caption with only whitespace lines falls through to filename parsing."""
    p = parse_filename("Some.Movie.2020.mkv", text="\n  \n  \n")
    assert p.category == "movie"
    assert p.year == 2020


# ── _parse_caption series header without episode line ──


def test_parse_caption_series_header_no_episode():
    """Series header matched but no episode line falls through to filename parsing."""
    p = parse_filename(
        "file.mp4",
        text="\u200e\U0001f3ac \u0633\u0631\u06cc\u0627\u0644 Test Show \u0645\u062d\u0635\u0648\u0644 \u0633\u0627\u0644 2024\n\U0001f4cc Some other line",
    )
    assert p.title is not None


# ── parse_filename empty stem ──


def test_parse_filename_empty_stem():
    """File with empty stem (just extension) returns category 'unknown'."""
    p = parse_filename(".mp4")
    assert p.category == "other"


# ── build_final_path forced series no stem ──


def test_build_final_path_forced_series_no_stem(monkeypatch):
    """Forced series category with no parsed stem synthesizes S01E01."""
    with tempfile.TemporaryDirectory() as td:
        monkeypatch.setattr(config, "DOWNLOAD_DIR", td)
        monkeypatch.setattr(config, "ORGANIZE_MEDIA", True)
        _path, fname = build_final_path("randomfile.mp4", base_dir=td, forced_category="series")
