...
# ── FloodWait retry that itself fails ──


def test_flood_wait_retry_fails():
    """First call raises FloodWaitError, retry raises RuntimeError → propagates."""
    attempt = {"n": 0}
...
def test_rate_limit_sleep():
    """Two rapid calls with a small _TG_MIN_INTERVAL both succeed (second waits)."""
    results = []
    ...
    async def _run():
        ...
        throttle._last_call = 0.0
        throttle._seq = 0
        throttle._TG_MIN_INTERVAL = 0.05
        throttle.start_publisher()
        try:
            t = Target()
            r1 = await throttle.send_message(t, "first")
            r2 = await throttle.send_message(t, "second")
            return r1, r2
        finally:
            throttle.stop_publisher()
